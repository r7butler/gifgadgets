...
async function pngFile(page,color='red') {
  const b64=await page.evaluate(color=>{const c=document.createElement('canvas');c.width=4;c.height=2;const ctx=c.getContext('2d');ctx.fillStyle=color;ctx.fillRect(0,0,4,2);return c.toDataURL().split(',')[1];},color);
