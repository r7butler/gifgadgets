...
class TestBuildSharePage:
    ...
    def test_contains_site_links(self):
        ...
        assert h.SITE_BRAND in html
        assert "Make Your Own" in html

    def test_brand_is_not_hardcoded(self, monkeypatch):
        """Share pages are generated by Lambda, so they miss a frontend rebrand."""
        monkeypatch.setattr(h, "SITE_BRAND", "ExampleBrand")
        html = h._build_share_page("Test", "https://cdn.com/gif.gif", "slug")
        assert "ExampleBrand" in html
        assert "GifWidgets" not in html
