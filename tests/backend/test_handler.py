...
class TestTrackSubmit:
    def test_feature_disabled(self, monkeypatch):
        ...
        event = make_event("/api/track/submit", {"frames": []})
        resp = h.handler(event, None)
        assert resp["statusCode"] == 503
    ...
    def test_successful_proxy(self, mock_urlopen):
        mock_response = MagicMock()
        mock_response.read.return_value = json.dumps({"masks": [1, 2, 3]}).encode()
        mock_response.__enter__ = lambda s: s
        mock_response.__exit__ = MagicMock(return_value=False)
        mock_urlopen.return_value = mock_response

        event = make_event("/api/track/submit", {"frames": ["f1", "f2"]})
        resp = h.handler(event, None)
        assert resp["statusCode"] == 200
        body = json.loads(resp["body"])
        assert body["masks"] == [1, 2, 3]
